...
class AConvLSTMModel(nn.Module):
    """
    Conv3D feature encoder + stacked AConvLSTM + 1×1 readout (used by experiment_runner.py).

    Input:  (B, T, 1, H, W)  — T past frames (e.g. P)
    Forward: runs the full sequence through the encoder and ConvLSTM; returns LSTM outputs and last states.
    predict_future: autoregressive rollout for Q steps from the last encoded frame channel and LSTM states.
    """
    ...
    def __init__(
        ...
        self.encoder = nn.Sequential(
            nn.Conv3d(in_channels, conv3d_channels, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm3d(conv3d_channels),
            nn.ReLU(inplace=True),
        )

        self.lstm = AConvLSTMLayers(
    ...
    def _encode_sequence(self, x):
        ...
        feat = self.encoder(x3d)
        return feat.permute(0, 2, 1, 3, 4)

    def _encode_frame(self, x_4d):
        """Single frame x: (B, 1, H, W) → (B, conv3d_channels, H, W) via T=1 Conv3D."""
        x5 = x_4d.unsqueeze(1)  # (B, 1, 1, H, W)
        return self._encode_sequence(x5)[:, 0, :, :, :]

    def forward(self, input_tensor):
        feat_seq = self._encode_sequence(input_tensor)
        return self.lstm(feat_seq)

    def predict_future(self, last_state_list, Q, first_input):
        """
        last_state_list: per-layer (h, c) after processing the input sequence
        Q: number of steps
        first_input: (B, 1, H, W) last raw frame — encoded before each LSTM step
        """
        cur_input = first_input
        predictions = []
        hidden_states = [(h.clone(), c.clone()) for (h, c) in last_state_list]
        ...
        for _ in range(Q):
            ...
            for layer_idx in range(self.lstm.num_layers):
                h, c = hidden_states[layer_idx]
                h, c = self.lstm.cell_list[layer_idx](
                    X=x,
                    H_prev=h,
                    C_prev=c,
                )
